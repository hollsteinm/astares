...
const f32 Particle::NoDamping = 0.999f;

Particle::Particle(const Vector3& velocity, const Vector3& acceleration, const Vector3& position, const f32& mass) :
...
Particle& Particle::SetMass(const f32& newMass) {
...
void Particle::Tick(f32 DeltaTime) {
	if (IsInfiniteMass()) {
		return;
	}
...
f32 Particle::GetMassReciprocal() const {
