...
TEST_CASE("Vector2", "[vectors]") {
	...
	SECTION("Data") {
		SECTION("Buffer") {
			f32 buff[2] = { 0.0f, 0.0f };
			Vector2 vec(5.0f, 3.4f);
	...
	SECTION("Mathematicial Operators") {
		SECTION("Vectors") {
			SECTION("Addition") {
				...
				Vector2 second(5.0f, 4.0f);

				Vector2 third = first + second;
				REQUIRE(third[_X] == 6.0f);
				REQUIRE(third[_Y] == 6.0f);

				Vector2 fourth = first + third;
		...
		SECTION("Real Numbers") {
			SECTION("Addition") {
				Vector2 vec(5.0f, -1.0f);
				Vector2 result = vec + 4.0f;
		...
		SECTION("Vector Math") {
			...
			Vector2 quote_inverted_quote = vec.GetInverse();
